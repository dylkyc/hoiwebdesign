...
* 中间是**模块配置区**，排布照原版 `tank_designer_view.gui` 的 `equipment_modules` 容器：
  **上排 6 个槽位图标**（炮塔 / 主炮 / 特殊槽 1-4）→ **蓝图** → **下排 3 个槽位 + 引擎、装甲两个等级步进器**
  （游戏里 `pos_custom_module_slot_window_0..8` 就是把这 9 个 76×47 的按钮钉在这三条带子上）。
  槽位按钮有模块剪影图标、槽位名、当前模块短名，左上角是「必 / 选」角标，
  配置有问题时右上角亮 `!`（对应原版的 `icn_requirements` / `icn_warning`）
* **蓝图会随配置变化**：车体、炮塔、炮管、负重轮全是纯 CSS 画的，换炮塔会改炮塔大小、换主炮会改炮管长度，
  固定战斗室的炮塔还会把炮塔换成斜置战斗室；四角是各槽位当前模块的标签
* **点槽位弹出模块选择窗**（照原版 `module_selector_window`）：两列卡片，
  按模块类别分组，每张卡是「图标 + 名称 + 年份 + 属性增减」
* 引擎 / 装甲可以用 `–` `+` 直接升降级（原版 `equipment_upgrade_0 / _1`）
